...
def send_flow(i,srcid,dstid):
    ...
    print(f"INNTER_ARRIVAL_TIME[{i}]={INNTER_ARRIVAL_TIME[i]} \t NUM_PKT[{i}]={NUM_PKT[i]*len(flow[i][(srcid,dstid)])}")
...
def main():
    ...
    for i,slicedict in flow.items():
        for e,v in slicedict.items():
                ...
                dstid=e[1]
                if len(v) != 0: 
                    clientdict[i] = Process(target = send_flow, args = (i,srcid,dstid))
