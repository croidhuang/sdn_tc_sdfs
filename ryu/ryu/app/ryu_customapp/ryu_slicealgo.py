...
#algo start
def slice_algo(topo_G, SliceNum, EDGE_BANDWIDTH_G, HISTORYTRAFFIC, SliceDraw_ctrl, ROUTING_TYPE, EXP_TYPE): 
    #initialze 
    allo_G = topo_G.copy()
    ...
    if SliceDraw_ctrl == True:
        ...
        nx.draw_networkx_edges(allo_G, topo_pos)
        for i in range(7):
            nx.draw_networkx(allo_G, topo_pos, nodelist=topo_sliceG[i].nodes(),edgelist = topo_sliceG[i].edges(),node_color="tab:green", edge_color = node_dist_to_color[i+1], font_color="whitesmoke", width = 4)
        nx.draw_networkx_edge_labels(allo_G, topo_pos, edge_labels = labels)
