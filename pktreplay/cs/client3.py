...
print(f"client {str(hostid)}: ready")
...
def pc_flow(pkt, i):
    ...
    chk_socket=num_to_hostipv4(slice_to_server(i))+":"+str(src_port)+"-"+num_to_hostipv4(slice_to_client(i))+":"+str(dst_port)
...
def sendp_flow(i, sendp_count):
    ...
    if pkt_ctrl[i][pkt_socket] == 1:
        try:
            flow[i][j][IP].src = num_to_hostipv4(slice_to_client(i))
            flow[i][j][IP].dst = num_to_hostipv4(slice_to_server(i))
